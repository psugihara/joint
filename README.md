...
To give a peek at the power and simplicity of Spliff, we first present a simple connection logger. Our logger program will write to stdout when a new user connects.
...
If we visit [http://localhost:8080/](http://localhost:8080/) and enter our name, `ourName arrived` will be printed on the console. We can record the activity log by routing stdout to a file: 
	...
	$ spliff logger.js 8080 static > activity.log
...
We assume basic familiarity with JavaScript and HTML but again there are a few important lines specific to Spliff that the reader should note. Namely, the first `script` tag includes the client side *pass.js* file which gives access to `server` functions inside of `pass.connect()`'s callback.
